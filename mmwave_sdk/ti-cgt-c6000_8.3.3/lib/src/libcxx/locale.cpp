...
#define _LIBCPP_LCONV_NO_C99_INTERNATIONAL_PREFIX
#elif defined(_LIBCPP_COMPILER_TI)
#define _LIBCPP_LCONV_NO_C99_INTERNATIONAL_PREFIX
                    ...
                    codecvt_mode mode = codecvt_mode(0))
{
    ...
    for (size_t nchar32_t = 0; frm_nxt < frm_end && nchar32_t < mx; ++nchar32_t)
    {
        ...
        else if (c1 < 0xF0)
        {
            ...
            if ((c3 & 0xC0) != 0x80)
                break;
            uint32_t t = (((static_cast<uint32_t>(c1) & 0x0Fu) << 12)
    ...
    for (; frm_nxt < frm_end - 1 && to_nxt < to_end; ++to_nxt)
    {
        ...
        else
        {
            ...
            if ((c2 & 0xFC00) != 0xDC00)
                return codecvt_base::error;
            uint32_t t =
                   (((((static_cast<uint32_t>(c1) & 0x03C0) >> 6) + 1) << 16)
                 |    ((static_cast<uint32_t>(c1) & 0x003F) << 10)
                 |     (static_cast<uint32_t>(c2) & 0x03FF));
            if (t > Maxcode)
                return codecvt_base::error;
            *to_nxt = t;
            frm_nxt += 4;
...
moneypunct_byname<char, true>::init(const char* nm)
{
    ...
    if (lc->int_frac_digits != CHAR_MAX)
        ...
        __frac_digits_ = base::do_frac_digits();
#ifdef _LIBCPP_LCONV_NO_C99_INTERNATIONAL_PREFIX
    if (lc->p_sign_posn == 0)
#else // _LIBCPP_MSVCRT
