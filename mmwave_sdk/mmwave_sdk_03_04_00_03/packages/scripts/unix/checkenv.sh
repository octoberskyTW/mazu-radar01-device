...
# Supported devices
devices_str="awr14xx, awr16xx, awr18xx, awr68xx, iwr14xx, iwr16xx, iwr18xx or iwr68xx"
...
if [ -z $MMWAVE_SDK_INSTALL_PATH ]; then
    ...
    printInfoMsg  "  export MMWAVE_SDK_INSTALL_PATH=~/ti/mmwave_sdk_03_04_00_03/packages"
    retval=1
fi
...
if [[ "${MMWAVE_SDK_DEVICE}" != @(awr14xx|iwr14xx|awr16xx|iwr16xx|awr18xx|iwr18xx|awr68xx|iwr68xx) ]]; then
...
if [ "$MMWAVE_SDK_DEVICE" == "awr16xx" ] || [ "$MMWAVE_SDK_DEVICE" == "iwr16xx" ] ||
   ...
   [ "$MMWAVE_SDK_DEVICE" == "awr68xx" ] || [ "$MMWAVE_SDK_DEVICE" == "iwr68xx" ]; then
if [ -z $C674_CODEGEN_INSTALL_PATH ]; then
    printErrorMsg "Error: C674_CODEGEN_INSTALL_PATH [DSP Toolchain] needs to be defined"
    printInfoMsg  "For example:"
    printInfoMsg  "  export C674_CODEGEN_INSTALL_PATH=~/ti/ti-cgt-c6000_8.3.3"
    retval=1
fi
...
if [ "$MMWAVE_SDK_DEVICE" == "awr68xx" ] || [ "$MMWAVE_SDK_DEVICE" == "iwr68xx" ]; then
if [ -z $XWR68XX_RADARSS_IMAGE_BIN ]; then
    ...
    printInfoMsg  "For example:"
    printInfoMsg  "  export XWR68XX_RADARSS_IMAGE_BIN=$MMWAVE_SDK_INSTALL_PATH/../firmware/radarss/xwr6xxx_radarss_rprc.bin"
    retval=1
else
