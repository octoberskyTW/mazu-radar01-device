Polymer({ is : "ti-rov-panel",
    ...
    ready : function () {
        ...
        this.canRestoreViews = !this.ccs7 ||
                (window.CCS_VERSION && Number(window.CCS_VERSION.substr(0, 1)) >= 8);
    },
    ...
    dashboardFileLoaded : function (file, closeAllViews, e) {
        var valid = false;
        var error = null;
        try {
            ...
            if (rovUtils.isArray(dashboard)) {
                if (dashboard[0].moduleName || dashboard[0].elemName) {
                    ...
                    if (closeAllViews) {
                        ...
                        this.showDashboard(dashboard, name, false);
                    }
                }
    ...
    dashboardFileReader : function (file, closeAllViews) {
        var reader = new FileReader();
        reader.onload = this.dashboardFileLoaded.bind(this, file, closeAllViews);
    ...
    initPlotViewDataContextMenu : function (dataProps) {
        if (this.plotViews.length == 0) {
            ...
            if (plots != null) {
                ...
                this.$.plotViewDataContextDialog.matched = plots;
            }
            else {
                this.$.plotViewData.hidden = false;
                this.$.addToGraph.hidden = true;
    ...
    listDashboardsDialog : function(dlgName, event) {
        ...
        if (dlgName == 'Save') {
            ...
            if (event.currentTarget.id.indexOf('overflow') == 0) {
                ...
                dialog.style.right = '';
                dialog.style.left = (event.x - 24) + 'px';
    ...
    matchPlotViews : function (dataProps) {
        ...
        for (var i = 0; i < this.plotViews.length; i++) {
            ...
            for (var j = 0; j < this.plotViews[i].traces.length; j++) {
                ...
                if (dataProps.module == trace.module &&
                    dataProps.view == trace.view &&
                    dataProps.primaryKeyColumnName == trace.primaryKeyColumnName &&
                    dataProps.primaryKeyColumnValue == trace.primaryKeyColumnValue &&
                    dataProps.selectedColumnName == trace.selectedColumnName) {
    ...
    newSavedView : function (viewProps, viewType) {
        if (viewType == 'module') {
            var view = this.newModuleView(viewProps.moduleName, viewProps.viewName,
                                          viewProps.viewsData, viewProps.viewArgs);
        }
        ...
        if (this.isADragDropFile(e)) {
            ...
            if (files.length == 1) {
                if (this.rovModuleViews.length + this.rovViews.length > 0) {
                    ...
                    this.dashboardFileReader(files[0], true);
                }
            }
            else {
                this.dashboardConflicts = [];
    ...
    openingDashboardAlertDialogButtonClicked : function (e) {
        ...
        if (this.$.openingDashboardAlertDialog.rovFile == null &&
            ...
            this.dashboardFileReader(this.$.openingDashboardAlertDialog.rovFile, closeAllViews);
        }
    },
    ...
    plotViewDataContextMenu : function (dataProps) {
        ...
        dialog.resetFit();
        dialog.style.left = dataProps.event.x + 'px';
        dialog.style.top = dataProps.event.y + 'px';
        this.initPlotViewDataContextMenu(dataProps);
    ...
    removeTraceFromPlotViewData : function (graphElem, traceProps) {
        ...
        for (var i = 0; i < this.plotViews.length; i++) {
            if (this.plotViews[i].elem.id == graphElem.id) {
                for (var j = 0; j < this.plotViews[i].traces.length; j++) {
                    var trace = this.plotViews[i].traces[j];
                    if (traceProps.module == trace.module &&
    ...
    restoreViewsChecked : function (e) {
        ...
        this.rovInit.restoreViews = e.currentTarget.checked;
        rovUtils.setLocalStorageObject('rovInit-storage', this.rovInit);
    },
    restoreViewsState : function () {
        var stateArr = rovUtils.getLocalStorageObject('currentViews-storage');
        for (var i = 0; i < stateArr.length; i++) {
            if (stateArr[i].moduleName) {
                if (this.allModules.indexOf(stateArr[i].moduleName) >= 0) {
                    ...
                    this.showStatus('Module ' + stateArr[i].moduleName + ' is not present in ' + document.title, 'warning');
                }
            }
    ...
    rovOptionsClicked : function(e) {
        ...
        if (dialog) {
            this.$.startupOptionsSubMenu.close();
            dialog.open();
            dialog.style.zIndex = String(this.getMaxZindex() + 1);
...
//            this.$.storageOptionsDialog.disabled = !this.hasViewsStorage();
    ...
    rovOptionsItemSelected : function (e) {
        if (e.currentTarget.selectedItem &&
            e.currentTarget.selectedItem.id.indexOf('Clicked') > 0) {
    ...
    saveViewsState : function (e) {
        this.rovInit.isNarrow = this.$.rovDrawerPanel.narrow;
        rovUtils.setLocalStorageObject('rovInit-storage', this.rovInit);
        var stateArr = [];
        for (var i = 0; i < this.rovModuleViews.length; i++) {
            var elem = this.getViewById(this.rovModuleViews[i].id);
        ...
        rovUtils.setLocalStorageObject('currentViews-storage', stateArr);
    },
    ...
    storageOptionsClicked : function (e) {
        var dialog = this.$.manageStorageDialog;
        dialog.resetFit();
        var viewsStorage = rovUtils.getLocalStorageObject('viewsData-storage');
        var keys = Object.keys(viewsStorage);
        var nonNullKeys = [];
    ...
    toolbarResponseToResize : function (narrow) {
        ...
        if (!narrow) {
            ...
            if (panelWidth < 300) {
                ...
                this.$.dashboardOverflowDialogButton.style.right = '33px';
    ...
    viewFromGraph : function(moduleName, viewName, viewArgs) {
            ...
            if (elem.moduleName == moduleName && elem.viewName == viewName) {
                if (viewArgs) {
                    if (elem.argsId && viewArgs[elem.argsId]) {
                        found = viewArgsCompare(viewArgs[elem.argsId], elem);
                        if (found) {
                            break;
                        }
                    }
        ...
        if (found) {
            ...
            var newElem = this.newModuleView(moduleName, viewName, false, viewArgs);
            newElem.style.zIndex = String(this.getMaxZindex() + 1);
            var key = moduleName + '.' + viewName;
            var viewsData = rovUtils.getLocalStorageObject('viewsData-storage');
            if (viewsData[key] && viewsData[key].width) {
