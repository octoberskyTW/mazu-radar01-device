...
void MainWindow::on_pushButton_start_clicked()
{
   ...
   QString filenameText = currDir.path();
    ...
    {
#ifdef MMWAVE_PLATFORM_1642
      filenameText.append("/profiles/profile_2d_VitalSigns_20fps_Back.cfg");        // Reads from this path
#else
       filenameText.append("/profiles/xwr1443_profile_2d_VitalSigns_20fps_Back.cfg");        // Reads from this path
#endif
    ...
    {
        filenameText.append("/profiles/profile_2d_VitalSigns_20fps_Front.cfg");        // Reads from this path
        qDebug() << "Configuration File Path is %s/n"<<filenameText;
        thresh_breath = 10;
        thresh_heart = 0.1;
...
#else
        demoParams.totalPayloadSize_bytes = LENGTH_MAGIC_WORD_BYTES + 4 * (demoParams.numRangeBinProcessed +  LENGTH_DEBUG_DATA_OUT_FLOAT);
#endif
        qDebug() << "Total Payload size from the UART is :"<< demoParams.totalPayloadSize_bytes;
...
void MainWindow::processData()
{
  ...
  if (MagicOk == 1)
  {
...
#else
   quint32  globalCountOut     = parseValueUint32(data,INDEX_IN_GLOBAL_FRAME_COUNT, 8);  // Global Count
#endif
   ...
   float outReserved_1         = parseValueFloat(data, INDEX_IN_RESERVED_1, 8);
   float outReserved_2         = parseValueFloat(data, INDEX_IN_RESERVED_2, 8);
   float outReserved_3         = parseValueFloat(data, INDEX_IN_RESERVED_3, 8);
   ...
   outHeartNew_CM = alpha_heartRateCM*(heartRate_CM) + (1-alpha_heartRateCM)*outHeartPrev_CM;
   ...
   if ((outSumEnergyBreathWfm < outSumEnergyBreathWfm_thresh) || (maxRCS_updated < RCS_thresh) )
   {
       flag_Breathing = 0;
       BreathingRate_Out = 0;
       QPalette lcdpaletteNotBreathing = ui->lcdNumber_Breathingrate->palette();
...
//   ui->plot_RangeProfile->yAxis->rescale();
   ...
   ui->lcdNumber_Display1->setDigitCount(8);
   myString_xCorr.sprintf("%1.0f",   heartRate_xCorr);
   ui->lcdNumber_Display1->display(myString_xCorr);

   QString myString_FFT_4Hz;
   ui->lcdNumber_Display2->setDigitCount(8);
   myString_FFT_4Hz.sprintf("%1.0f",   heartRate_FFT_4Hz);
   ui->lcdNumber_Display2->display(myString_FFT_4Hz);

   QString myString_Reserved_1;
   ui->lcdNumber_Display3->setDigitCount(8);
   myString_Reserved_1.sprintf("%1.3f", outReserved_1);
   ...
   QString myString_Reserved_2;
   ui->lcdNumber_reserved2->setDigitCount(8);
   myString_Reserved_2.sprintf("%1.3f", outReserved_2);
   ui->lcdNumber_reserved2->display(myString_Reserved_2);

   QString myString_Reserved_3;
   ui->lcdNumber_reserved3->setDigitCount(8);
   myString_Reserved_3.sprintf("%1.3f", outReserved_3);
   ui->lcdNumber_reserved3->display(myString_Reserved_3);
